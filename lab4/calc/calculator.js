...
function clickHandler(event) {
    ...
    let screenSpan = document.querySelector(".screen_span");

    if (event.target.className == "key clear gray_btn") {
        screenSpan.innerHTML = '0';
        counter = 0;
    } else if (event.target.className == "key result orange_btn") {
        screenSpan.innerHTML = evaluate(screenSpan.innerHTML);
    } else { 
        if (counter == 0) {
            screenSpan.innerHTML = null;
            screenSpan.innerHTML += event.target.innerHTML;
            counter++
        } else {
            screenSpan.innerHTML += event.target.innerHTML;
        }
